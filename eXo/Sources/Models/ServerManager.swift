            ...
            for server in serverList {
                if (items.count < Config.maximumShortcutAllow) {
                    if (Config.communityURL.contains((server as! Server).serverURL.stringURLWithoutProtocol())) {
                        ...
                        items.add(UIApplicationShortcutItem.init(type: ShortcutType.connectRecentServer, localizedTitle: "Shortcut.Title.ConnnecteXoTribe".localized(), localizedSubtitle: nil, icon: UIApplicationShortcutIcon(templateImageName: "eXoTribeLogo"), userInfo: ((server as! Server).toDictionary() as! [AnyHashable: Any])))
                    } else {
                        // A common Logo for the other servers
                        items.add(UIApplicationShortcutItem.init(type: ShortcutType.connectRecentServer, localizedTitle: "Shortcut.Title.ConnectTo".localized(), localizedSubtitle: (server as! Server).natureName(), icon: UIApplicationShortcutIcon(templateImageName: "server"), userInfo: ((server as! Server).toDictionary() as! [AnyHashable: Any])))
                    }
                }
            }
        } else {
